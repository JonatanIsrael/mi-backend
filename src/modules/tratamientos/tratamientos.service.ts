...
export class TratamientosService {
  ...
  async crear(crearTratamientoDto: CrearTratamientoDto, userId: number) {
    if (!crearTratamientoDto.id_proyecto)
      throw new Error('id_proyecto es requerido');

    const proyecto = await this.proyectosService.encontrarPorId(
      crearTratamientoDto.id_proyecto,
      userId
    );

    const tratamiento = this.tratamientosRepo.create({
      ...crearTratamientoDto,
      proyecto,
    });
  ...
  async encontrarPorId(id: number, userId: number) {
    const tratamiento = await this.tratamientosRepo
      .createQueryBuilder('tratamiento')
      .innerJoinAndSelect('tratamiento.proyecto', 'proyecto')
      .innerJoin('proyecto.investigadorPrincipal', 'usuario')
      .where('tratamiento.id = :id', { id })
  ...
  async encontrarPorProyecto(idProyecto: number, userId: number) {
    return await this.tratamientosRepo
      .createQueryBuilder('tratamiento')
      .innerJoin('tratamiento.proyecto', 'proyecto')
      .innerJoin('proyecto.investigadorPrincipal', 'usuario')
